...
public class HealItemStoreMgr : MonoBehaviour
{
    ...
    {  //Count == 0 인 상태가 처음 나오는 아이템만 구매가능으로 표시해 준다.
        if (m_Item_ScrollContent != null)
        {
            if (m_CrNodeList == null || m_CrNodeList.Length <= 0)
                m_CrNodeList =
                    m_Item_ScrollContent.GetComponentsInChildren<HealItemNoodCtrl>();
        }
        //int a_FindAv = -1;
        for (int ii = 0; ii < GlobalUserData.m_ItemDataList.Count - 7; ii++)
        //for (int ii = 5; ii < 8; ii++)
        {
            ...
            if (5 > GlobalUserData.m_ItemDataList[ii].m_CurItemCount) //구입상태
            {
                m_CrNodeList[ii].SetState(HealItemState.Active);
            ...
            //    a_FindAv = ii;
            //}
            else if (5 == GlobalUserData.m_ItemDataList[ii].m_CurItemCount)
            {
                //Debug.Log(ii + "Lock");
                //전부 Lock 표시
                m_CrNodeList[ii].SetState(HealItemState.Lock);
            }
    ...
    { //(서버로부터 골드, 아이템 상태 받아와서 클라이언트와 동기화 시켜주기..)
        ...
        string a_Mess = "";
        HealItemState a_ItemState = HealItemState.Lock;
        ...
        {
            a_ItemState = m_CrNodeList[(int)m_BuyCrType ].m_HealItemState;
    ...
    public void TryBuyItem()  //구매 2단계 함수 
    {  //(서버에 전달할 변동된 데이터 값 만들기...)
        ...
        for (int ii = 0; ii < GlobalUserData.m_ItemDataList.Count; ii++)
        {
            ...
            if (ii != (int)m_BuyCrType || 5 <= a_ItemInfo.m_CurItemCount) //구매 조건 체크
                continue;

            int a_Cost = a_ItemInfo.m_Price * m_CurNum;
            ...
            a_SetLevel[ii] = m_CurNum + m_InitNum;     //레벨증가 백업해 놓기
